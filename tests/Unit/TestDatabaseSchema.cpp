...
{
    const auto& migrations = LibriFlow::DatabaseSchema::CDatabaseSchema::GetMigrationStatements();

    REQUIRE(LibriFlow::DatabaseSchema::CDatabaseSchema::GetCurrentVersion() == 1);
...
{
    const std::string_view script = LibriFlow::DatabaseSchema::CDatabaseSchema::GetCreateSchemaScript();
