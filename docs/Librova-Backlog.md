...
- when closing a task, in the same change remove the full four-line entry from `Open Backlog` and add the full four-line entry under the matching priority section in `## 4. Closed Backlog` with status `Closed`; replace the `Note` with a one-sentence summary of what was actually done (no exhaustive detail);
- after any backlog edit, verify there are no orphan task-title lines without `Status`, `Type`, and `Note`, and that no task id appears in both `Open Backlog` and `Closed Backlog`.
