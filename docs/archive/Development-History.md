...
- `2026-04-02` Feature: completed the dark media-app redesign for `Librova.UI`, including shared dark theme resources, refreshed shell/view styling, consistent action semantics, stable language-filter selection behavior, a new application icon, and updated manual validation guidance for the accepted UI behavior.
- `2026-04-03` Fix: split library bootstrap into explicit `open existing` vs `create new` modes across the UI and native host, blocked silent in-place recreation for damaged libraries, and added native plus managed regression coverage for startup recovery and shell library switching.
- `2026-04-03` Fix: made native host CLI path parsing and file logging Unicode-safe on Windows, switched native diagnostics to shared UTF-8 path conversion, and added regression coverage for Cyrillic path logging plus UTF-8 host option parsing.
- `2026-04-03` Fix: added an explicit UI-to-host graceful shutdown signal for normal desktop exit, kept parent-process watchdog shutdown for crash-like termination, and added native plus managed regression coverage for the new stop path.
- `2026-04-03` Fix: sanitized free-text FTS input before building SQLite `MATCH` queries so ordinary punctuation no longer turns user search into raw FTS syntax errors, with added repository regression coverage.
- `2026-04-03` Test: added strong host-backed UI regression coverage for `Export` and `Move To Trash` immediately after directory import, closing the stale manual-only reproduction gap for that flow.
- `2026-04-03` Refactor: replaced read-side `GetById()` loops in SQLite search and probable-duplicate detection with batched hydration of book rows, authors, and tags, and added repository regression coverage for result ordering, metadata completeness, and exact author-set matching.
- `2026-04-03` Fix: extended shared metadata normalization beyond Russian-only Cyrillic case folding so `І/і`, `Ї/ї`, `Є/є`, and `Ў/ў` stay case-insensitive in both SQLite search indexing and probable-duplicate detection, with dedicated repository regression coverage.
- `2026-04-03` Refactor: centralized native UTF-8, wide-string, legacy-code-page, and `std::filesystem::path` conversions under `libs/Unicode`, migrated remaining duplicate helpers to that slice, and added dedicated regression coverage for Cyrillic path round-trips plus Windows-1251 decoding.
- `2026-04-03` Fix: kept bootstrap UI logging outside the selected `Create Library` target until the shell session actually starts, preventing first-run startup from turning an empty chosen folder non-empty before native library creation validation runs.
- `2026-04-03` Fix: downscaled oversized imported cover images before writing them into managed storage, hid Windows/WIC image processing behind a swappable native interface, and added regression coverage for import-time cover optimization plus fallback behavior.
- `2026-04-03` Fix: replaced the `Library` browser's fixed first-page behavior with scroll-driven incremental loading, exposed total result counts for both unfiltered and filtered queries through the protobuf/native read side, and updated managed plus native regression coverage for the new browsing contract.
- `2026-04-03` Fix: sourced `Library` language-filter options from the full matching read-side result set instead of only already loaded cards, handled load-more faults as controlled UI errors, and refreshed deep-scroll delete results in a single range request, with updated managed plus native transport coverage and refreshed manual UI expectations.
- `2026-04-03` Feature: changed `Library` delete to stage files through managed `Trash`, then hand them off to the Windows `Recycle Bin`, returning an explicit delete-destination result over protobuf so the UI can surface managed-`Trash` fallback when Windows handoff fails, with updated native plus managed regression coverage and refreshed manual verification guidance.
- `2026-04-03` Fix: cleaned up empty managed-`Trash` subfolders left behind after successful Windows `Recycle Bin` handoff, with added native regression coverage for the post-delete directory cleanup.
- `2026-04-03` Build: added the canonical `scripts/PublishPortable.ps1` Windows packaging flow, taught the UI to resolve a packaged `LibrovaCoreHostApp.exe` beside `Librova.UI.exe`, introduced the `x64-release-static` preset plus static MSVC runtime alignment for a single-file native host, redirected portable bootstrap UI state, preferences, and pre-library logs into `PortableData/` beside the packaged app, and verified the resulting portable output through a release smoke test. The current verified package layout is `Librova.UI.exe`, `LibrovaCoreHostApp.exe`, and the required Avalonia native runtime DLLs.
