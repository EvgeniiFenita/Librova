...
## 9.2 Settings Version

1. Перейди в секцию `Settings`.
Ожидаемое поведение:
- виден отдельный блок `Application`;
- в нем показана строка `Version` с текущей версией приложения;
- версия отображается без пустого значения и без технических placeholder-строк.
