...
- если converter path не настроен в текущей сессии, кнопка `Export As EPUB` для `FB2` видна, но неактивна;
...
Ожидаемое поведение:
- в MVP UI нет отдельного поля для YAML-конфига `fbc.yaml`;
