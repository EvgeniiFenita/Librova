...
Реестр всех сценариев ручной проверки UI. Детальные сценарии разбиты по файлам в [`docs/manual-tests/`](manual-tests/); этот файл хранит правила запуска и таблицу-реестр.

Используй эти сценарии после user-visible UI или runtime изменений и перед тем, как считать checkpoint завершенным.

- сценарии с `RG = ✓` обязательны перед релизом
- сценарии без `RG` используй как целевой smoke / regression check для измененной области
...
Колонка **RG** (Release Gate) показывает сценарии, обязательные перед каждым релизом. Пустая колонка означает, что сценарий не является обязательным релизным gate, но остается полезной целевой ручной проверкой.
...
| [library.md](manual-tests/library.md) | 2.3 | Фасетная панель фильтров (Languages + Genres) | |
