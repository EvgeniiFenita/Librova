...
- native Librova code in C++20;
- Qt/QML application code under `apps/Librova.Qt`;
...
- Keep domain logic independent from UI, database, and OS-specific code.
...
- **Infrastructure utility functions that operate on state or resources (database connections, file handles, OS handles) must be grouped as `static` methods of a `final` class with a `C` prefix — not as free functions in a namespace.** Free functions are acceptable only for pure stateless transformations (e.g. math helpers, string formatting). When in doubt, use a class.
...
- Important execution paths in both C++ and Qt code must have actionable logs at startup, shutdown, UI/backend boundaries, long-running operations, and failure paths.
...
**UI/backend boundary logging rules** (mandatory):

- Every Qt adapter wrapping an `ILibraryApplication` call must log the successful response outcome and route backend exceptions through explicit error paths. Success callbacks must not run after backend exceptions.
- Log level guide:
  - `Info` / `Debug` — expected outcomes, normal state transitions, UI/backend results on happy path.
...
- **Never construct `std::filesystem::path` from `std::string`, `const char*`, or `std::string_view` that carries UTF-8 content.** The constructor `std::filesystem::path(std::string)` on Windows uses the system ANSI codepage, silently corrupting non-ASCII paths (e.g. Cyrillic). Use `PathFromUtf8()` for every UTF-8→path conversion.
...
- Keep QML focused on presentation, navigation state, and view composition.
- Keep business rules in native application/domain layers or QML-facing Qt adapters and controllers.
- Reuse shared QML controls and `LibrovaTheme` / `LibrovaTypography` tokens instead of hardcoded per-view styling.
- QML IDs use lower camel case with a leading underscore only for private implementation objects already following that local pattern.
- UI-facing C++ controllers, adapters, and models follow the same C++ naming and include rules as the rest of the repository.
- Long-running backend calls must go through the backend dispatcher rather than blocking the GUI thread.
...
- Standard ecosystem files keep their canonical names, for example `CMakeLists.txt`, `.gitignore`, `CMakePresets.json`.
- Documentation files use `PascalCase` without underscores.
- QML files use `PascalCase.qml` for reusable components and page/section types.
