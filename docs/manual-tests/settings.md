...
4. Нажми `Open Library...` и выбери такую же вложенную папку внутри текущей библиотеки.
Ожидаемое поведение:
...
4. Введи абсолютный путь к **несуществующему** файлу (например, `C:\nonexistent\fbc.exe`).
Ожидаемое поведение:
...
4.1. Введи абсолютный путь к **существующему рабочему** `fbc.exe`.
Ожидаемое поведение:
- кратковременно показывается нейтральный статус `Validating converter…`;
- кнопка `Save` недоступна во время проверки;
- после успешной пробы статус исчезает, ошибок нет;
- кнопка `Save` становится активной.
4.2. Нажми `Browse...` рядом с built-in executable path и выбери `fbc.exe`.
Ожидаемое поведение:
...
4.3. Быстро введи несколько символов подряд (имитируй набор нового пути).
Ожидаемое поведение:
...
4.4. Проверь built-in `fb2cng` block целиком.
Ожидаемое поведение:
