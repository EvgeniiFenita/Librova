...
Ожидаемое поведение:
- под путем к библиотеке виден summary `Library: ...` с общим числом managed books и единым total library size в `MB`, включая `Objects` и `Database/librova.db`.
2. Нажми `Open` и выбери другой существующий library root.
Ожидаемое поведение:
...
2.1. Повтори `Open`, но выбери обычную непустую папку без структуры Librova.
Ожидаемое поведение:
- переключение библиотеки не начинается;
- validation error объясняет, что `Open` требует существующую библиотеку Librova.
3. Нажми `New` и выбери новый пустой путь.
Ожидаемое поведение:
- Librova создает managed library structure в новом месте.
- приложение переключается на новую библиотеку как на активную.
3.1. Повтори `New`, но выбери обычную непустую папку.
Ожидаемое поведение:
- переключение библиотеки не начинается;
- validation error объясняет, что `New` требует пустую целевую папку.
3.2. Повтори `New` с новым пустым путем, содержащим кириллицу.
Ожидаемое поведение:
- библиотека создается успешно;
- diagnostics и `host.log` продолжают показывать этот путь корректным Unicode-текстом.

## 1.1 Open Or Create Library Rejects Nested Current-Library Folders

1. Открой существующую библиотеку Librova и перейди в `Settings`.
2. Нажми `New`.
3. В системном picker выбери вложенную папку внутри текущей библиотеки, например `Trash` или `Objects\23\7d`.
Ожидаемое поведение:
- приложение не принимает такой путь как новую библиотеку;
- показывается понятная validation error, что nested subfolder текущей библиотеки нельзя использовать как отдельную library root.
4. Нажми `Open` и выбери такую же вложенную папку внутри текущей библиотеки.
Ожидаемое поведение:
- приложение не пытается переключиться на эту папку;
- пользователь получает явную ошибку валидации вместо тихого перехода в некорректное состояние.

## 2. Settings: Converter Configuration

1. Открой `Settings`.
2. Введи относительный путь к executable.
Ожидаемое поведение:
- показывается converter validation error.
- сохранение недоступно.
3. Введи абсолютный путь к **несуществующему** файлу (например, `C:\nonexistent\fbc.exe`).
Ожидаемое поведение:
- кратковременно показывается нейтральный статус `Validating converter…`;
- кнопка `Save` недоступна во время проверки;
- после завершения пробы появляется сообщение об ошибке (файл не найден или probe не вернул EPUB);
- сохранение по-прежнему недоступно.
3.1. Введи абсолютный путь к **существующему рабочему** `fbc.exe`.
Ожидаемое поведение:
- кратковременно показывается нейтральный статус `Validating converter…`;
- кнопка `Save` недоступна во время проверки;
- после успешной пробы статус исчезает, ошибок нет;
- кнопка `Save` становится активной.
3.2. Нажми `Browse...` рядом с executable path и выбери `fbc.exe`.
Ожидаемое поведение:
- путь к `fbc.exe` подставляется в поле автоматически;
- путь остается абсолютным;
- автоматически запускается probe-проверка: кратко виден `Validating converter…`;
- если `fbc.exe` рабочий — после успешной пробы ошибок нет, `Save` активна.
3.3. Быстро введи несколько символов подряд (имитируй набор нового пути).
Ожидаемое поведение:
- probe не запускается на каждое нажатие клавиши;
- видна только одна проверка (debounce ~500 мс), которая запускается после паузы в наборе.
3.4. Проверь converter block целиком.
Ожидаемое поведение:
- в текущем UI нет выбора converter mode;
- на экране есть только путь к `fbc.exe`, кнопка `Browse...`, статус валидации и `Save`.
4. Нажми `Save`.
Ожидаемое поведение:
...
5. Через `fbc.exe` выполни импорт `FB2` с включенным `Force conversion to EPUB during import`, затем выполни `Export as EPUB` для другой `FB2` книги.
Ожидаемое поведение:
