...
## 1.3 Import Validation: Async Source Validation Clears Before Start

1. Открой раздел `Import`.
2. Нажми `Select Files...` и выбери один валидный `.fb2` или `.epub`.
Ожидаемое поведение:
- сразу после выбора файла источник принимается, начинается фоновая проверка источника;
- если валидация ещё не завершилась, `Start Import` недоступен;
- как только валидация завершается без ошибок, импорт стартует автоматически без дополнительного нажатия.
3. Повтори сценарий через `Select Folder...` с папкой, содержащей supported файлы.
Ожидаемое поведение:
- аналогично шагу 2: import стартует автоматически после завершения валидации.

## 1.4 Import Validation: Empty Directory Produces Warning At Import Time

1. Открой раздел `Import`.
2. Нажми `Select Folder...` и выбери пустую папку (либо папку, содержащую только неподдерживаемые файлы, например `.txt`).
Ожидаемое поведение:
- validation message НЕ блокирует запуск: директория принимается на этапе валидации (шалоу-проверка, без рекурсивного обхода);
- import стартует автоматически;
- по завершении итоговый summary показывает 0 imported, warning о том, что директория не содержала поддерживаемых файлов.
...
- drag-and-drop zone визуально затемнена вместе с кнопками `Select Files...` / `Select Folder...`;
- перетаскивание новых файлов или папок в окно не показывает доступный drop-target и не подменяет текущий running source в UI.
