# LibriFlow Architecture Master Document

## 1. Overview

LibriFlow is a desktop application for managing a personal library of e-books. The main reference point is Calibre, but the target product is intentionally narrower for the first release: simpler scope, clearer architecture, stronger separation between UI and core logic, and a test-driven native backend.
...
LibriFlow should be implemented as a two-process desktop system:
...
LibriFlow follows Clean Architecture with clear boundaries:
...
- LibriFlow is a library manager, not a reading application.
